...
const AdminPanel = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (formData.examType === 'present' && (!formData.lastDate || !/\d{4}/.test(formData.lastDate))) {
  ...
  const tabCategories = [...new Set(tabExams.map(e => e.category).filter(Boolean))];
  const activeExams = selectedCategory ? tabExams.filter(e => e.category === selectedCategory) : tabExams;
      ...
      <div className="admin-panel">
          ...
          <div className="admin-form-container">
            ...
            <form onSubmit={handleSubmit} className="admin-form">
              ...
              <div className="form-row">
                ...
                {/* Present → Last Date | Upcoming → Notification Expected Date */}
                {formData.examType === 'present' ? (
                  <div className="form-group">
                  ...
                  <div className="form-group">
                    <label className="form-label">Notification Expected Date</label>
                    <input type="text" name="notificationDate" value={formData.notificationDate} onChange={handleChange} className="form-input" placeholder="e.g. 2026 / July 2026 / 20 July 2026" />
                  </div>
                )}
              </div>
        ...
        {activeSection === 'exams' && (
          <div className="admin-exams-list">
            ...
            <div className="saved-tabs">
              <button className={`saved-tab ${activeExamTab === 'present' ? 'active' : ''}`} onClick={() => { setActiveExamTab('present'); setSelectedCategory(''); }}>
                Present Exams <span className="tab-count">{presentExams.length}</span>
              </button>
              <button className={`saved-tab ${activeExamTab === 'upcoming' ? 'active' : ''}`} onClick={() => { setActiveExamTab('upcoming'); setSelectedCategory(''); }}>
                Upcoming Exams <span className="tab-count">{upcomingExams.length}</span>
              </button>
            </div>

            <div className="admin-filter-row">
              <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="admin-category-select">
              ...
              {selectedCategory && (
                <button className="clear-filter-btn" onClick={() => setSelectedCategory('')}>✕ Clear</button>
              )}
            </div>
              ...
              <table>
                <thead>
                  <tr>
                    <th>Title</th>
                    <th>Category</th>
                    <th>{activeExamTab === 'upcoming' ? 'Notification Date' : 'Last Date'}</th>
                    <th>Official Link</th>
                    <th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {activeExams.length === 0 ? (
                    <tr><td colSpan="5" style={{ textAlign: 'center', padding: '1.5rem', color: 'var(--text-secondary)' }}>No exams found</td></tr>
                  ) : (
                    activeExams.map(exam => (
                      <tr key={exam._id}>
                        <td>{exam.title}</td>
                        <td>{exam.category}</td>
                        <td>{activeExamTab === 'upcoming' ? (exam.notificationDate || 'Not specified') : (exam.lastDate || 'Not specified')}</td>
        ...
        {activeSection === 'users' && (
          <div className="admin-exams-list">
            ...
            <div className="saved-tabs">
              <button className={`saved-tab ${activeUserTab === 'all' ? 'active' : ''}`} onClick={() => setActiveUserTab('all')}>All <span className="tab-count">{users.length}</span></button>
              <button className={`saved-tab ${activeUserTab === 'male' ? 'active' : ''}`} onClick={() => setActiveUserTab('male')}>Male <span className="tab-count">{maleUsers.length}</span></button>
              <button className={`saved-tab ${activeUserTab === 'female' ? 'active' : ''}`} onClick={() => setActiveUserTab('female')}>Female <span className="tab-count">{femaleUsers.length}</span></button>
              <button className={`saved-tab ${activeUserTab === 'other' ? 'active' : ''}`} onClick={() => setActiveUserTab('other')}>Other <span className="tab-count">{otherUsers.length}</span></button>
            </div>
            <div className="admin-table">
              <table>
                <thead>

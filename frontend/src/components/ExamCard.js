...
const formatSmartDate = (dateStr) => {
  ...
  if (/^[A-Za-z]+\s+\d{4}$/.test(str)) {
    ...
    if (monthIndex !== undefined) {
      return new Date(year, monthIndex).toLocaleDateString("en-IN", {
        year: "numeric",
        month: "short",
      });
    }
...
const ExamCard = ({ exam, onSaveToggle }) => {
  ...
  const isUpcoming = exam.examType === 'upcoming';
      ...
      <div>
        ...
        {isUpcoming ? (
          <p className="exam-date">
            <strong>Notification Expected:</strong>{" "}
            <span className="last-date">
              {formatSmartDate(exam.notificationDate)}
            </span>
          </p>
        ) : (
          <p className="exam-date">
